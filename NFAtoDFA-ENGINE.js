...
class NFA {
    ...
    toDotString() {
        ...
        dotStr += "INITIAL_STATE -> " + this.formatDotState(this.initialState) + ";\n";
...
function lambdaClosureNFA(nfa) {
    ...
    for (let i = 0; i < nfa.states.length; i++) {
        ...
        // 2) Figure out the next state for each state in the state_closure for each symbol in the alphabet
    ...
    let initial_state_closure = fetch_E_Closure(nfa.initialState, nfa.transitions);
...
function generateDFA(nfa, step_counter_stop = -1) {
            ...
            if (combinedStatesUnion != null) {
                ...
                if (!dfa_states.includes("TRAP")) {
                    ...
                    for (let n = 0; n < nfa.alphabet.length; n++)
                        dfa_transitions.push(new Transition("TRAP", ["TRAP"], nfa.alphabet[n]));
    ...
    return new NFA(nfa.initialState, dfa_final_states, dfa_states, nfa.alphabet, dfa_transitions);
...
function minimizeDFA(dfa) {
    ...
    for (let state of dfa.states) {
        ...
        for (let state2 of dfa.states) {
            ...
            if (state !== state2 && (dfa.finalStates.includes(dfa.formatDotState(state)) === dfa.finalStates.includes(dfa.formatDotState(state2)))) {
                ...
                for (let symbol of dfa.alphabet) {
                    ...
                    let state1_nextStates = findNextStates(state, symbol, dfa.transitions);
                    let state2_nextStates = findNextStates(state2, symbol, dfa.transitions);
